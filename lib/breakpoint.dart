...
class Breakpoint {
  ...
  factory Breakpoint.fromConstraints(
    ...
    if (constraints.debugAssertIsValid()) {
      ...
      _orientation = constraints.maxHeight > constraints.maxWidth
  ...
  factory Breakpoint.fromMediaQuery(
    ...
    final double _width = _media.size.width;
    final Orientation _orientation = _media.orientation;
  ...
  static int useColumns(int availableColumns, int divisor) {
    ...
    int _availableColumns = availableColumns;
    int _divisor = divisor;

    if (_availableColumns < 1 || _availableColumns > 24) _availableColumns = 1;

    if (_divisor < 1 || _divisor > 24) _divisor = 1;
    return (_availableColumns / _divisor).floor() < 1
        ? 1
        : (_availableColumns / _divisor).floor();
  }
  ...
  static int usedColumns({
    ...
    int _numberOfColumns = numberOfColumns ?? 1;
    if (_numberOfColumns < 1 || _numberOfColumns > 24) _numberOfColumns = 1;
    ...
    if ((columnSpace == null || columnSpace == 1) &&
        (minColumnSize == null ||
            minColumnSize <= 137 / 2 ||
            minColumnSize > 8192)) return _numberOfColumns;

    // If columnSpace input was invalid, we use 1
    int? _columnSpace = columnSpace;
    if (_columnSpace != null && (_columnSpace < 1 || _columnSpace > 24)) {
      _columnSpace = 1;
    }
    ...
    if (_columnSpace != null) {
      ...
      columnsViaColumnInterval = (_numberOfColumns / _columnSpace).floor() < 1
          ? 1
          : (_numberOfColumns / _columnSpace).floor();
    }
    ...
    double? _width = width;
    if (_width == null || _width < 50) _width = 50;
    ...
    if (minColumnSize != null) {
      ...
      columnsViaMinSize = (_width / minColumnSize).floor() < 1
          ? 1
          : (_width / minColumnSize).floor();
    }
